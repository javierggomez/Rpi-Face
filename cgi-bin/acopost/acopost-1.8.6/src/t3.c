...
#define FILE_SEMAPHORE "/var/www/cgi-bin/data/semaphore.t3"
...
typedef struct globals_s
{
  ...
  char *wf;     /* name of file to write output */
  int bw;       /* beam width */
...
/* ------------------------------------------------------------ */
word_pt new_word(char *s, int cnt, int not)
{
...
void get_options(globals_pt g, int argc, char **argv)
{
  ...
  while ((c=getopt(argc, argv, "a:b:dhl:m:qr:s:tuv:xyz"))!=255)
    {
      switch (c)
	{
	case 'a':
  ...
  if (optind+3<argc && strcmp("-", argv[optind+3]))
    { g->wf=strdup(argv[optind+3]); }
}

/* ------------------------------------------------------------ */
/* previously inlined */
int find_tag(model_pt m, char *t)
{ return (int)hash_get(m->taghash, t)-1; }

/* ------------------------------------------------------------ */
int register_tag(model_pt m, char *t)
{
  int i=find_tag(m, t);
...
/* ------------------------------------------------------------ */
/* previously inlined */
int ngram_index(int n, int s, int t1, int t2, int t3)
{
  switch (n)
    {
    case 0: return t1;
  ...
  for (lno=0, s=freadline(f); s; lno++, s=freadline(f))
    {
      ...
      if (1!=sscanf(s, "%d", &cnt)) { error("can't read count (%s:%d)\n", g->mf, lno); }
...
#define DEBUG_COMPUTE_COUNTS_FOR_BOUNDARY 0
  ...
  {
    int bXb=0, bXY=0, XYb=0, bbX=0, bX=0, Xb=0, i, j, k, c1, c2, c3;
...
/* ------------------------------------------------------------ */
float tag_sentence(model_pt m, array_pt words, array_pt tags, char *l, FILE *outFile, int *count)
{
  char *t;
  int i;
  float result=0;
...
/* ------------------------------------------------------------ */
float tagging(model_pt m)
{
  float average=0;
  int count=0;
  report(1, "Started tagging file\n");
  FILE *f= g->rf ? try_to_open(g->rf, "r") : stdin;
  FILE *outFile=g->wf ? try_to_open(g->wf, "w"): stdout;  
  ...
  for (l=freadline(f); l; l=freadline(f))
    { average+=tag_sentence(m, words, tags, l, outFile, &count);
     }
  array_free(words); array_free(tags);
  if (outFile!=stdout) { fclose(outFile); }
  if (f!=stdin) { fclose(f); }
  report(1, "Finished tagging file\n");
...
void testing(model_pt m)
{
  ...
  for (l=freadline(f); l; l=freadline(f))
      ...
      for (t=strtok(l, " \t"), i=0; t; t=strtok(NULL, " \t"), i++)
	{
      ...
      for (i=0; i<array_count(words); i++)
	{
	  int guess=(int)array_get(tags, i);
	  int ref=(int)array_get(refs, i);
	  if (guess==ref) { pos++; }
	  else
	    {
	      if (verbosity>3)
		  ...
		  for (j=i-2; j!=i; j++)
		    {
		      ...
		      report(-1, "%s %s ", (char *)array_get(words, j),
			     (char *)array_get(m->tags, (int)array_get(refs, j)));
		    }
...
void loop(model_pt m) {
	while (1) {
		while (access(FILE_SEMAPHORE, F_OK)) {
			usleep(10000);	
		}
		float average=tagging(m);
		usleep(10000);
		unlink(FILE_SEMAPHORE);
    report(1, "Media calculada: %f\n", average);
    char *command=(char*)mem_malloc(256);
    sprintf(command, "/var/www/cgi-bin/move_face %f", average);
    system(command);
    mem_free(command);
	}
}
